  ...
  def main(args: Array[String]): Unit = {
    ...
    Storage.using { implicit s =>
      if (evaluation.isEmpty) {
        ...
        val engineInstanceId = s.getMetaDataEngineInstances().insert(engineInstance)
        ...
        CoreWorkflow.runTrain(
          ...
          engineInstances = s.getMetaDataEngineInstances(),
          metadata = s.getModelDataModels())
      } else {
        val workflowParams = WorkflowParams(
          verbose = wfc.verbosity,
          skipSanityCheck = wfc.skipSanityCheck,
        ...
        Workflow.runEvaluation(
          ...
          evaluationInstances = s.getMetaDataEvaluationInstances(),
          params = workflowParams)
      }
    }
  }
